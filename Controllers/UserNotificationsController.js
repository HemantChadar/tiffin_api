const { getUserNotificationById, getUserNotificationByToken, getUserNotification, addUserNotification, updateUserNotification } = require("../Models/UserNotifications")
...
exports.getUserNotification = async (req, res) => {
    ...
    try {
        if (req?.body?.id) {
            data = await getUserNotificationById(req.body.id);
            if (data?.length >= 1) {
            ...
            data = await getUserNotificationByToken(req.body.token);
            if (data?.length >= 1) {
                res.json(successResponse("data successfully get", data[0]))
            } else {
...
exports.addUserNotification = async (req, res) => {
    let data = [];
    try {
        data = await addUserNotification(req.body);
        const user = await getUserNotificationById(data.insertId);
...
exports.updateUserNotification = async (req, res) => {
    ...
    let user = [];
    try {
        data = await updateUserNotification(req.body, keyName, keyValue);
        if (req.body.id) {
            user = await getUserNotificationById(req.body.id);
        }
        if (req.body.token) {
            user = await getUserNotificationByToken(req.body.token);
        }  
