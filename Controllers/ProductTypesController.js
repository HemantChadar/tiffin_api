const { getProductTypeById, getProductTypeByToken, getProductType, addProductType, updateProductType } = require("../Models/ProductTypes")
...
exports.getProductType = async (req, res) => {
    ...
    try {
        if (req?.body?.id) {
            data = await getProductTypeById(req.body.id);
            if (data?.length >= 1) {
            ...
            data = await getProductTypeByToken(req.body.token);
            if (data?.length >= 1) {
                res.json(successResponse("data successfully get", data[0]))
            } else {
...
exports.addProductType = async (req, res) => {
    let data = [];
    try {
        data = await addProductType(req.body);
        const user = await getProductTypeById(data.insertId);
...
exports.updateProductType = async (req, res) => {
    ...
    let user = [];
    try {
        data = await updateProductType(req.body, keyName, keyValue);
        if (req?.body?.id) {
            user = await getProductTypeById(req.body.id);
        }
        if (req?.body?.token) {
            user = await getProductTypeByToken(req.body.token);
        }  
