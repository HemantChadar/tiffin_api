const { getOrderProductTypeById, getOrderProductTypeByToken, getOrderProductType, addOrderProductType, updateOrderProductType } = require("../Models/OrderProductTypes")
...
exports.getOrderProductType = async (req, res) => {
    let data = [];
    try {
        if (req?.body?.id) {
...
exports.addOrderProductType = async (req, res) => {
    let data = [];
    try {
        data = await addOrderProductType(req.body);
        const user = await getOrderProductTypeById(data.insertId);
...
exports.updateOrderProductType = async (req, res) => {
    let keyName = ""
    let keyValue = null
    if (req?.body?.id) {
        keyName = "id"
    ...
    try {
        data = await updateOrderProductType(req.body, keyName, keyValue);
        if (req?.body?.id) {
            user = await getOrderProductTypeById(req.body.id);
        }
        if (req?.body?.token) {
            user = await getOrderProductTypeByToken(req.body.token);
        }
