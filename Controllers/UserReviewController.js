...
exports.addUserReview = async (req, res) => {
    let data = [];
    try {
        data = await addUserReview(req.body);
        const user = await getUserReviewById(data.insertId);
...
exports.updateUserReview = async (req, res) => {
    ...
    let user = [];
    try {
        data = await updateUserReview(req.body, keyName, keyValue);
        if (req?.body?.id) {
            user = await getUserReviewById(req.body.id);
        }
        if (req?.body?.token) {
