const { getUserById, getUserByToken, getUser, addUser, updateUser } = require("../Models/Users")
...
exports.getUser = async (req, res) => {
    ...
    try {
        if (req?.body?.id) {
            data = await getUserById(req.body.id);
            if (data?.length >= 1) {
            ...
            data = await getUserByToken(req.body.token);
            if (data?.length >= 1) {
                res.json(successResponse("data successfully get", data[0]))
            } else {
...
exports.updateUser = async (req, res) => {
    ...
    let user = [];
    try {
        data = await updateUser(req.body, keyName, keyValue);
        if (req?.body?.id) {
            user = await getUserById(req.body.id);
        }
        if (req?.body?.token) {
            user = await getUserByToken(req.body.token);
        }  
